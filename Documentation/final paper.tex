...
Machine Learning is a very broad topic with much of contemporary computer science based on it or consisting of research done around it. The main technique that is being applied is that of a Support Vector Machine or SVM. This maximises the distance between two clusters.

\subsubsection*{Set Up}

The set up of data and input revolved around two functions. These are 'getPeaks' and 'getTroughs'. They take input of date, time, stock, and X, where X is the number of data points that are being used for learning. 'getPeaks' will take this input, sub-sample the data and find all points in the data that would have been optimal to sell at, that is all the peaks. 'getTroughs' will do the opposite, it will find all points at which it would have been optimal to buy. This data is then stored in tables 'SVMBuyData' and 'SVMSellData'. These are in the form:
...
ADD ALL TECHNIQUES USED HERE

\subsubsection*{Query}

The query of the SVM is fairly simple, a new data point will be added and using the pre-learned model, it will be decided which group it will fall into. 

TWO QUERY FUNCTIONS? YES

This is all managed by the 'Query' function, taking input if date, time, and stock, whilst the number of data points to learn from will be predetermined to provide optimal performance. The 'Query' function will then create the data on which the SVM will be trained using 'getPeaks' and 'getTroughs' and using the built in training function will create a fully trained SVM which will then be queried using the built in query functions that will return a decision
